...
void Entities::GetInfo()
{
	...
	for (int i = 1; i < amount; i++)
	{
		...
		ReadProcessMemory(winFunc.processHandle, LPCVOID(list[i].base + AR_Ammo), &list[i].ar_ammo, sizeof(list[i].ar_ammo), nullptr);
		ReadProcessMemory(winFunc.processHandle, LPCVOID(list[i].base + SMG_Ammo), &list[i].smg_ammo, sizeof(list[i].smg_ammo), nullptr);
		ReadProcessMemory(winFunc.processHandle, LPCVOID(list[i].base + Sniper_Ammo), &list[i].sniper_ammo, sizeof(list[i].sniper_ammo), nullptr);
		ReadProcessMemory(winFunc.processHandle, LPCVOID(list[i].base + Shotgun_Ammo), &list[i].shotgun_ammo, sizeof(list[i].shotgun_ammo), nullptr);
		ReadProcessMemory(winFunc.processHandle, LPCVOID(list[i].base + CB_Ammo), &list[i].carbine_ammo, sizeof(list[i].carbine_ammo), nullptr);
		ReadProcessMemory(winFunc.processHandle, LPCVOID(list[i].base + Grenade), &list[i].grenade_ammo, sizeof(list[i].grenade_ammo), nullptr);
